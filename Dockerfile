...
RUN chmod -R 777 /var/www/localhost/htdocs/ && sed -i '/<Directory \/var\/www\localhost/htdocs/>/,/<\/Directory>/ s/AllowOverride None/AllowOverride All/' /etc/apache2/httpd.conf \
&& sed -i 's/\\.php$>/"\\.php$|\\.php5$|\\.php4$|\\.php3$|\\.phtml$|\\.phpt$">/' /etc/apache2/conf.d/php.conf